...
void main() {
    float res_x = 320.0;
    float res_y = 240.0;
    vec2 dx = vec2(1.0 / vec2(res_x, res_y).x, 0.0);
    vec2 dy = vec2(0.0, 1.0 / vec2(res_x, res_y).y);
    vec2 pixCo = fragTexCoord * vec2(res_x, res_y) - vec2(0.5);
    vec2 texCo = (floor(pixCo) + vec2(0.5)) / vec2(res_x, res_y);
    ...
    float scanWeight = 1.0 - pow(cos(fragTexCoord.y * 2.0 * PI * vec2(res_x, res_y).y) * 0.5 + 0.5, scanBeam) * 1.0;

    float mask = 1.0 - 0.3;
    vec2 modFac = floor(fragTexCoord * vec2(640.0, 480.0) * vec2(res_x, res_y) / (vec2(res_x, res_y) * vec2(1.0, 1.0 * 1.0)));
    ...
    if (vec2(res_x, res_y).y >= 400.0) scanWeight = 1.0;
...
// void main() {
//     vec2 texSize = vec2(res_x, 240.0); // your internal resolution
