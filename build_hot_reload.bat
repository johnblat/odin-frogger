...
if not exist %BUILD_DIR% mkdir %BULD_DIR%
...
:: unique name on each hot reload.set /p PDB_NUMBER=<%GAME_PDBS_DIR%\pdb_number
...
echo Building %EXE%
odin build src\main_hot_reload -debug -out:%OUT_DIR%\%EXE% -pdb-name:%OUT_DIR%\main_hot_reload.pdb
IF %ERRORLEVEL% NEQ 0 exit /b 1
